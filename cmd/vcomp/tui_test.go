...
func TestTUIWorkflowInRealTerminal(t *testing.T) {
	...
	keys("m")
	wait("user message form", func() bool { return has("Send message FROM USER to ceo") })
	keys("Enter")
	typeText("Late designer")
	keys("Enter", "Down", "Enter")
	typeText("Please coordinate with the new designer.")
	keys("C-s")
	wait("user message delivered", func() bool { return has("Message delivered:") })
	inbox, _ := os.ReadDir(filepath.Join(root, "spaces", "ceo", "inbox"))
	if len(inbox) != 1 || !strings.HasPrefix(inbox[0].Name(), "URGENT - FROM USER - Late designer - ") {
		t.Fatalf("message missing from CEO inbox: %v", inbox)
	}
	// Inspect and edit the actual shared definition, then delete and restore it.
	keys("7", "Enter")
	wait("profession definition", func() bool { return has("profession definition") && has("Your remit") })
	keys("e")
	wait("profession edit scope", func() bool { return has("Edit profession") && has("My defaults") })
	keys("C-s")
	wait("profession draft", func() bool { return has("/ draft") && has("Draft only") })
	keys("s")
	wait("profession save confirmation", func() bool { return has("Save ") && has("global scope?") })
	keys("y")
	wait("profession saved", func() bool { return has("Profession saved in global scope") })
	keys("d")
	wait("profession delete scope", func() bool { return has("Delete from catalogue") })
	keys("C-s")
	wait("profession delete confirmation", func() bool { return has("Existing employees keep") })
	keys("y")
	wait("profession deleted", func() bool { return has("Profession deleted from catalogue") })
	keys("z", "Home", "u")
	wait("profession restore scope", func() bool { return has("Restore profession") })
	keys("C-s")
	wait("profession restored", func() bool { return has("Profession restored.") })
	keys("p")
	wait("authoring settings", func() bool { return has("Role generation settings") && has("claude-opus-5") })
	keys("Escape", "1")
	os.Remove(filepath.Join(root, "editor-visited"))
	keys("c")
	...
	keys("y")
	wait("terminal restored", func() bool { b, err := os.ReadFile(filepath.Join(root, "after")); return err == nil && len(b) > 0 })
